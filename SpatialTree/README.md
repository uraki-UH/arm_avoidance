...
`query_aabb(min_point, max_point, visitor)`で、各軸の閉区間に入る要素を取得可能。非有限値・逆転した上下限は例外。空間分割は検索用であり、点座標の量子化や占有判定ではない。

```cpp
tree.query_aabb(min_point, max_point, [&](const MyNode *node) {
    // 指定範囲内の元要素の利用
});
```

登録点がルート領域内にあり、セル所属と座標が一致していることが前提。上記の微小移動時の索引更新省略やヒステリシスでセル外へはみ出した点への適用は対象外。Tmapの静的検索では更新時に必要な範囲で再構築し、TF変化には検索箱の変換で対応。
