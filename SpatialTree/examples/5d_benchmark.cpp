...
template <int Dim>
Point<double, Dim> sample_hypersphere_surface(std::mt19937 &gen, double radius) {
...
int main() {
  ...
  std::cout << "Starting 5D Pure GNG Performance Benchmark..." << std::endl;

  GNGParams<double> g_params;
  g_params.lambda = 100000000;
  g_params.max_nodes = 1000000;

  SpatialTreeParams<double> s_params;
  s_params.max_nodes_per_cell = 200;
  s_params.min_cell_size = 0.0;

  GrowingNeuralGas<double, Dim, LazyNeighborUpdate, false> gng(
      Point<double, Dim>(100.0, 100.0, 100.0, 100.0, 100.0), g_params, s_params);
  ...
  std::cout << "Starting 5D Pure GNG training (100k nodes)..." << std::endl;
