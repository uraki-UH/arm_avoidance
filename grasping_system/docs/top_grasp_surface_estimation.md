...
ハッシュ集合への操作を平均定数時間とした見積もりであり、実測時間ではない。評価対象候補数を$M$として上方障害物の確認に$O(MN)$、付属探索には候補ごとの訪問済み表初期化$O(N)$と到達部分の幅優先探索のコストが追加。参照平面モードでは探索先ごとに参照面との距離を確認。累積距離制限はなく、探索量はグラフの接続と判定条件に依存。
