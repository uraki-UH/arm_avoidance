...
「上方」は候補TCPの進入方向の制約に加え、平面法線と`up_axis`の傾斜角で判定。法線の符号反転を許容し、C++既定の許容角は25度。現在の`ToPoDualArm.yaml`では旧方式に近い設定へ戻し、許容角90度・非平面付属判定OFF・上方障害物判定OFF。ゼロ・不正法線の除外は維持。
...
- 距離近傍探索やクラスタ全対比較は不要。
- 非平面ノードとのエッジは平面間の直接隣接には不使用。付属成分の接続判定には利用。
