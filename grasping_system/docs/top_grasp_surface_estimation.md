...
2. 平面ノードを距離0の始点として、同一成分内のエッジを距離上限付きDijkstraで探索。平面OBB外側余白・下方深さ・上方高さの範囲内だけを採用。`BOUNDARY_FREE_SPACE`を持つ端点は探索を通過させない。境界ビットは観測根拠であり、物体端の確定ではない。
...
| `max_nonplane_graph_dist` | `0.15 m` | 累積エッジ長 |
...
ハッシュ集合への操作を平均定数時間とした見積もりであり、実測時間ではない。追加した判定では、評価対象候補数を$M$として上方障害物の確認に$O(MN)$、付属探索には候補ごとの距離表初期化$O(N)$と局所Dijkstraのコストが追加。探索距離・空間範囲で制限するが、定数時間やノード密度不変ではない。
