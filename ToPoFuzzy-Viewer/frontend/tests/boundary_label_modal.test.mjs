...
try {
    ...
    assert.equal((html.match(/を先頭へ移動"/g) ?? []).length, 10);
    ...
    assert.equal((html.match(/を長押しして移動"/g) ?? []).length, 10);
    assert.equal((html.match(/border-slate-600 bg-slate-800\/40 hover:border-slate-400/g) ?? []).length, 10);
    assert.equal((html.match(/data-drag-indicator/g) ?? []).length, 10);
    assert.equal((html.match(/lucide-hand-grab/g) ?? []).length, 10);
    ...
    assert.equal(elements.filter((item) => item.type === 'details').length, 2);
    ...
    assert.equal(elements.filter((item) => item.props.type === 'checkbox').length, 8);
    assert.equal(elements.filter((item) => item.props.type === 'color').length, 8);
    ...
    elements.find((item) => item.type === 'label' && item.props.children.includes('視野端')).props.children[0].props.onChange({ target: { checked: false } });
