...
接続・`request.state`時にサーバーが対応能力を通知。

```json
{"type":"stream.capabilities","marker_array_applied":true}
```

対応クライアントは`{"type":"stream.marker_array.ready"}`を返信し、以後はトピックごとに未反映Markerを1件まで配信。
React反映後の描画機会を待って`{"type":"stream.marker_array.applied","topic":"/plane_clusters/markers/nodes"}`を返信。
待機中の入力はサーバー側で最新1件だけ保持し、再開時に完全なスナップショットを送信。中間フレームの全件配信は非保証。
Frontendも同一描画周期のMarkerを最新1件へ集約。ノード・エッジの間引きや件数制限の追加なし。

- 対応能力の通知がない旧サーバーへは新通知を送信しない。`ready`を返さない旧クライアントは反映通知不要。
- Markerごとの待機はGraphと独立。ソケット送信待ち解消時にも最新フレームを再送判定。
- `request.state`は反映待ちでなければ最新Markerを再配信。反映待ちへの重複送信なし。
- 購読解除・配信元停止・全接続終了では待機とキャッシュを破棄。再接続・再購読後の最新状態復元。
- 共有スタイルを含む完全スナップショットのため、中間フレーム省略による辞書欠落なし。
...
ROS Marker用には同じ値をlinear RGBへ変換。辞書は他の矢印スタイルと同じ配信・再接続規約。

- 同一設定の矢印は同じ`arrow_style_id`を参照。
- `arrow_styles`の省略は直前の辞書を保持。存在する場合は辞書全体を置換。`{}`は空辞書への更新。
- 現行サーバーは各スナップショットに完全な辞書を付与。旧サーバーの辞書省略も受信可能。レイヤー削除時は辞書も破棄。
