...
int main() {
    try {
        ...
        require(gng_setParameter("input.voxel_grid_unit", 0, 0.5f), "入力ボクセル設定の受理");
#else
        require(!gng_setParameter("input.voxel_grid_unit", 0, 0.5f), "入力ボクセル設定の拒否");
#endif
#ifdef GNG_USE_NODE_GRID
        require(gng_setParameter("node.grid", 0, 0.5f), "ノードグリッド設定の受理");
#else
        require(!gng_setParameter("node.grid", 0, 0.5f), "ノードグリッド設定の拒否");
#endif
...
#ifdef GNG_USE_INPUT_VOXELS
        require(statistics.num_training_points == 4, "重複と近接点の重心への統合");
        require(statistics.num_zero_samples == 0 && statistics.voxel_ms > 0, "重心からの学習とボクセル処理時間");
#else
        require(statistics.num_training_points == 6, "元点抽出候補の保持");
        require(statistics.num_zero_samples > 1000 && statistics.voxel_ms == 0, "重複原点も入力比率どおり選択");
#endif
        require(statistics.attention_ms == 0 &&
