...
struct TopologicalMapStorage{
    vector<TopologicalNode> nodes;
    vector<TopologicalCluster> clusters;
    vector<uint16_t> edges;
    vector<uint32_t> nodes_inpcl_ids; // ノードに対応する入力点群IDの配列
    vector<uint16_t> clusters_nodes; // 排他集合
    vector<Vec3f> input_pcl; // 入力点群データ
    vector<uint8_t> inpcl_labels; // 入力点群のラベル
    uint32_t node_num;
    uint32_t cluster_num;
    uint32_t edge_num;
    uint32_t input_pcl_num;
    void init(size_t node_num_max, size_t cluster_num_max, size_t edge_num_max, size_t inpcl_num){
        nodes.resize(node_num_max);
        clusters.resize(cluster_num_max);
        edges.resize(edge_num_max * 2);
        nodes_inpcl_ids.resize(inpcl_num);
        clusters_nodes.resize(node_num_max);
        input_pcl.resize(inpcl_num);
        inpcl_labels.resize(inpcl_num);
        node_num = 0;
        cluster_num = 0;
        edge_num = 0;
        input_pcl_num = 0;
...
class GNG{
    ...
    vector<uint32_t> input_point_ids; // YAML範囲内の元点番号。入力順・重複点を保持。
    ...
    input_voxels voxel_input;
#endif
    Labelling la;
    Clustering cl;

    // output
    TopologicalMapStorage map;
