...
MY_API int gng_setParameter(const char *paramerter_name, const uint32_t index, const float value){
    ...
    if (name == "node.enable_observation_support") {
        ...
        // メモリ配置・座標範囲・既存学習係数の再構築が必要な設定の動的変更抑止。
        if (gng.initialized && (name == "node.num_max" || name == "node.grid" ||
            name == "node.eta_s1" || name == "node.eta_s2" ||
            name == "edge.num_max" || name == "cluster.num_max" ||
            name.rfind("input.", 0) == 0)) {return false;}
        if (!gng.param.setParameter(paramerter_name, index, value)) {return false;}
        if (gng.initialized) {
            core.gng_config = gng.param.node;
            core.setTrainingEventMaxWinnerRank(core.training_event_winner_rank_max);
        }
        return true;
    }
...
extern "C" MY_API gng_efficiency_view gng_get_efficiency_view() {
    return {gng_efficiency_ms, gng.vg.voxel_index.data(), gng.vg.voxel_range.data(),
        ...
        static_cast<uint32_t>(gng.attention_pcl_num), gng.n1.trial_learning_num};
}
extern "C" MY_API double gng_get_voxel_sort_ms() {return gng.vg.sort_ms;}
extern "C" MY_API void gng_set_trial_seed(uint32_t seed) {gng.n1.trial_seed = seed;}
extern "C" MY_API void gng_set_trial_last_representative(uint8_t enable) {
    gng.n1.enable_trial_last_representative = enable != 0;
}
extern "C" MY_API void gng_set_trial_input_capture(uint8_t enable) {
    gng.n1.enable_trial_input_capture = enable != 0;
    gng.n1.trial_inputs.reserve(gng.param.node.learning_num);
}
extern "C" MY_API const Vec3f *gng_get_trial_inputs(uint32_t *num) {
    *num = gng.n1.trial_inputs.size();
    return gng.n1.trial_inputs.data();
}
