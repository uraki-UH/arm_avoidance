...
void CUGNG::learn(vector<Vec3f> &inpcl, int input_pcl_num, vector<Vec3f> &attention_pcl, int attention_pcl_num,
    ...
    const auto learn_input = [&](int idx) {
        if (raw_points && source_voxels && has_observation_frame_origin) {
            ...
            const auto &range = source_voxels->voxel_range[idx];
            // 検証用コピー限定の代表点切替。学習座標・乱数列への変更なし。
            const auto representative_idx = enable_trial_last_representative ? range.end - 1 : range.start;
            const auto raw_idx = source_voxels->voxel_index[representative_idx].raw_index;
    ...
    mt19937 mt(trial_seed + frame_number); // 比較用シードとフレーム番号。
#else
...
void CUGNG::learn_normal(Vec3f& p, const Vec3f *observation_point, uint32_t raw_idx, bool enable_statistics) {
    ++trial_learning_num;
    if (enable_trial_input_capture) {trial_inputs.push_back(p);}
    static Node_d n;
    int i;
    // 全探索
