...
struct NodeConfig{
    ...
    int max_static_s1_age; // 長期記憶ノードの未観測・未選択寿命（GNG更新回数）
};

struct EdgeConfig{
    int num_max; // エッジ数の上限
    uint32_t age_max;        // Edgeの年齢による切断の閾値
    float distance_min2[4]; // エッジの距離の最小値
};
...
struct ClusterConfig{
    int num_max; // クラスタ数の上限
    size_t node_num_min;  // クラスタリングの個数の閾値
