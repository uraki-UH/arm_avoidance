...
void Param::init() {
    ...
    edge.num_max = 300000;
    edge.age_max = 100;
    edge.distance_min2[0] = 0 * 0; // default
    edge.distance_min2[1] = 0.2 * 0.2;  // safe (clusted)
    edge.distance_min2[2] = 0.2 * 0.2;// wall (clusted)
    edge.distance_min2[3] = 0 * 0; // unknown (clusted)
...
bool Param::setParameter(const char *p_c, const uint32_t index, const float value) {
    string p(p_c);
    // 最小版で未使用の設定への明示的な拒否。値だけを受理する挙動の防止。
#ifndef GNG_USE_NODE_GRID
    if (p == "node.grid") {return false;}
#endif
#ifndef GNG_USE_INPUT_VOXELS
    if (p == "input.voxel_grid_unit") {return false;}
#endif
    if ((p == "node.grid" || p == "input.voxel_grid_unit") &&
        (!std::isfinite(value) || value <= 0)) {return false;}
    if (p == "node.unknown_learning_rate" || p == "node.s1_reset_range" ||
        p.rfind("sampling.", 0) == 0 || p.rfind("ds.", 0) == 0 ||
        p.rfind("cluster.", 0) == 0 || p.rfind("classify.", 0) == 0) {return false;}
    ...
    /* Node Config */
        ...
        edge.distance_min2[index] = edge_distance_min * edge_distance_min;
    } else if (p == "node.static.age_min") {
        node.static_age_min = value;
    } else if (p == "node.static.s1_age_max" && std::isfinite(value) && value > 0 &&
        std::floor(value) == value && static_cast<double>(value) < std::numeric_limits<int>::max()) {
        node.max_static_s1_age = static_cast<int>(value);
    } else if (p == "node.s1_age_max" && index < 4 && value > 0) {
        node.s1_age[index] = value;
    } else if (p == "node.clusted_s1_age" && index < 4 && value > 0) {
        node.clusted_s1_age[index] = value;
    /* Edge Config */
    } else if (p == "edge.num_max" && value >= 0) {
        edge.num_max = value;
    } else if (p == "edge.age_max" && value >= 0) {
        edge.age_max = value;
    /* Label Config */
    } else if (p == "label.fuzzy.unknown" && value >= 0)  {
        label.fuzz_unknown_1 = 1.f / value;
    ...
    /* Cluster Config */
    } else if (p == "cluster.num_max" && value >= 1) {
        cluster.num_max = value;
    } else if (p == "cluster.node_num_min" && value >= 1) {
        cluster.node_num_min = value;
