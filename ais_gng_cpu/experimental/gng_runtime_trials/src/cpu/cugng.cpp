...
bool CUGNG::init(NodeConfig *node_config, EdgeConfig *edge_params, OtherConfig *input_config) {
    ...
    vector<uint32_t> free_ids(node_num_max);
    std::iota(free_ids.begin(), free_ids.end(), 0);
    free_node_ids = decltype(free_node_ids)(std::greater<uint32_t>(), std::move(free_ids));
#endif
#ifdef GNG_USE_NODE_GRID
    Vec3f min_cell_pos = min_input_pos, max_cell_pos = max_input_pos;
    for (int axis = 0; axis < 3; ++axis) {
        min_cell_pos.p[axis] -= input_config->node_grid;
        max_cell_pos.p[axis] += input_config->node_grid;
    }
    if (!node_cells.init(min_cell_pos, max_cell_pos, input_config->node_grid)) {return false;}
    page_offsets.assign((static_cast<uint64_t>(node_cells.num_total) + num_cells_per_page - 1) / num_cells_per_page, UINT32_MAX);
    cell_counts.assign(node_cells.num_total, 0);
#else
...
void CUGNG::clear() {
#ifdef GNG_USE_NODE_GRID
    cell_nodes.clear(); page_offsets.clear(); cell_counts.clear();
#else
    spatial_index.reset();
