...
int main() {
    ...
    submit();
    gng_set_observation_origin({0.434f, -0.693f, 0.279f}, 1);
    gng_exec();
    ...
    // 次入力用原点と直近出力フレームの分離。
    gng_set_observation_origin({5, 6, 7}, 1);
    ...
    submit();
    gng_set_observation_origin({0, 0, 0}, 1);
    gng_exec();
    ...
    submit();
    gng_set_observation_origin({std::numeric_limits<float>::quiet_NaN(), 0, 0}, 1);
    gng_exec();
    ...
    submit();
    gng_set_observation_origin(config.pos, 1);
    gng_exec();
    ...
    submit();
    gng_set_observation_origin(config.pos, 1);
    require(gng_set_observation_pixels(pixel_ids.data(), pixel_ids.size(), table, 1), "pixel setup failed");
    gng_exec();
    auto lookup = gng_get_observation_lookup_statistics();
    ...
    gng_set_observation_origin(config.pos, 1);
    gng_exec();
    lookup = gng_get_observation_lookup_statistics();
    ...
    submit();
    gng_set_observation_origin(config.pos, 1);
    require(!gng_set_observation_pixels(pixel_ids.data(), pixel_ids.size() - 1, table, 1), "short pixel input accepted");
    std::fill(pixel_ids.begin(), pixel_ids.end(), UINT32_MAX);
    require(gng_set_observation_pixels(pixel_ids.data(), pixel_ids.size(), table, 1), "fallback setup failed");
    gng_exec();
    lookup = gng_get_observation_lookup_statistics();
    ...
    submit(); gng_set_observation_origin(config.pos, 1);
    require(gng_set_observation_pixel_view(&view, table, 1), "view setup failed");
    gng_exec();
    lookup = gng_get_observation_lookup_statistics();
    ...
    submit(); gng_set_observation_origin(config.pos, 1); gng_exec();
    require(gng_get_observation_lookup_statistics().pixel_hit_num == 0, "view persisted");
    require(supported_node_num(expected) > 0, "view fallback support missing");
    submit(); gng_set_observation_origin(config.pos, 1);
    require(gng_set_observation_pixel_view(&view, table, 1), "borrow setup failed");
    std::fill(pixel_ids.begin(), pixel_ids.end(), UINT32_MAX);
    gng_exec();
    lookup = gng_get_observation_lookup_statistics();
    ...
    submit(); gng_set_observation_origin(config.pos, 1);
    --view.data_size;
    require(!gng_set_observation_pixel_view(&view, table, 1), "short view accepted");
    require(!gng_set_observation_pixel_view(nullptr, table, 1), "null view accepted");
    gng_exec();
    ...
    for (const uint32_t point_num : {65536U, 65537U, 900U, 65537U, 900U}) {
        ...
        submit(); gng_set_observation_origin(config.pos, 1);
        require(gng_set_observation_pixel_view(&view, table, 1), "switch view setup failed");
        gng_exec();
        lookup = gng_get_observation_lookup_statistics();
