...
境界ノード周辺にも配分する[境界重点学習](boundary_attention.md)と併用可能。以下の通常復帰・配分例は把持重点単独の場合。併用時はそれぞれの有効な重点集合を維持。
...
3. GNG既存の入力範囲フィルタとセル索引を使用。AABBに完全に含まれるセルは元点走査なし、部分交差セルだけ実測XYZを判定。複数候補を一括したAABBは作らず、重複範囲の点も1回だけ選択。
4. [共通サンプラー](sampling.md)へ重みを渡し、境界等の条件と配分を統合。可視化購読時だけ共通候補を展開して配信。
...
ノードからの距離にかかわらず候補内部・余白内の実測点が対象。AABBは観測済み候補の広がりであって真の物体形状の確定値ではなく、近くの床や別物体の点も範囲に入る場合あり。範囲構築は所属ノード数に比例。全元点ではなくセル単位で候補範囲と照合し、詳細判定は部分交差セルへ限定。

## 選択点群の可視化

`/downsampling/grasp`（`sensor_msgs/msg/PointCloud2`）を購読。重点学習ON時のみPublisherを作成し、購読者がいる場合だけXYZを点群化。追加のON/OFF設定はなし。

- 内容は候補AABB＋余白内の重点入力候補。2026-09-25の共通化後はGNG入力範囲フィルタ適用後。最終的に各反復で抽選された点の順列や回数ではない。
