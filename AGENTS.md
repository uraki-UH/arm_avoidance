...
- `/goal` の実行中は、許可済みの範囲で実装・検証・修正を自律的に継続。通常の作業確認や重複した許可確認は不要。
- 承認が必要な操作は、利用可能な自動承認審査へ委任。審査の拒否や実行環境の制限を回避する操作は禁止。
- 既存データの一括削除、`git reset --hard`、強制pushなど、データや履歴を失う破壊的な操作は禁止。
...
- 報告文書は「要約」「条件・検証」の2節を基本とし、40〜80行を目安に要点・結果・必要条件へ限定。数値の対象・単位、失敗・未検証、互換性上の制約は保持。
- 同じ作業の続報は既存文書へ統合。「変更なし」だけの節、経緯の羅列、同じ数値表・仕様・コマンドの重複は省略し、詳細は正本へのリンクで参照。
- 仕様・設計・再現手順は利用に必要な詳細を保持。過去の報告は確認日を明記し、当時の状態と現行仕様を区別。
