...
RUN echo '. /opt/ros/humble/setup.bash' >> /root/.bashrc && \
    ...
    echo 'function cb() { cd /${ROS2_WS}; if [ $# -eq 0 ]; then colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release; else colcon build --symlink-install --packages-select "$@" --cmake-args -DCMAKE_BUILD_TYPE=Release; fi; . install/setup.bash && cd -; }' >> /root/.bashrc && \
