...
public:
  ...
  : Node("fuzzy_cluster_summary")
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Cluster summary node started: subscribe=%s summary_publish=%s box_publish=%s",
      input_topic_.c_str(),
      output_topic_.c_str(),
...
private:
  void publish_cluster_boxes(const ais_gng_msgs::msg::TopologicalMap::SharedPtr msg)
  {
    ...
    for (const auto & cluster : msg->clusters) {
      ...
      const auto color = label_color(cluster.label);
  ...
  void topological_map_cb(const ais_gng_msgs::msg::TopologicalMap::SharedPtr msg)
  {
    ...
    for (const auto & cluster : msg->clusters) {
      ...
      ++inferred_counts[cluster.label_inferred];
      ...
      if (include_cluster_details_) {
        ...
        row.label_inferred_raw = cluster.label_inferred;
        row.label = label_name(cluster.label);
        row.label_inferred = label_name(cluster.label_inferred);
