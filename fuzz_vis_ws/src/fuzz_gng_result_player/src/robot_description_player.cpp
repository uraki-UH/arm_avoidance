  ...
  : Node("robot_description_player") {
    description_file_ =
        declare_parameter<std::string>("robot_description_file",
                                       "/home/fuzzrobo/uraki_ws/fuzz_arm/topoarm_description/urdf/topoarm.urdf.xacro");
    mesh_root_dir_ = declare_parameter<std::string>(
        "mesh_root_dir",
        "/home/fuzzrobo/uraki_ws/fuzz_arm/topoarm_description/meshes/topoarm");
...
private:
  ...
  void tick() {
    ...
    if (!have_last_mtime_ || mtime != last_mtime_) {
      ...
      last_description_ = std::move(msg.data);
