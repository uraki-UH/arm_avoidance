...
checked = []
for method in ['before'] + (['before_pool'] if 'pooled' in args.methods else []) + args.methods:
    ...
    for name,(frames,warmup,enable_features) in expected.items():
        ...
        reference = 'before_pool' if method in ('before_pool', 'pooled') else 'before'
        before = json.loads((output/reference/(name+'.json')).read_text())
...
result = dict(methods=args.methods,cases=checked,executed_frames=sum(row['frames'] for row in checked),
              compared_frames=sum(row['frames'] for row in checked if row['method'] not in ('before','before_pool')))
