...
for trial in 1 2 3; do
  for variant in grid spatial_fixed_order; do
    OPENBLAS_NUM_THREADS=1 timeout -s INT -k 5 120 python3 "$benchmark_script" \
      --library "$comparison_dir/libgng_${variant}.so" \
      --config "$benchmark_config" \
      --bag "$benchmark_bag" \
      --output "$comparison_dir/final_${variant}_${trial}.json" \
      > "$comparison_dir/final_${variant}_${trial}.stdout" 2>&1
    tail -1 "$comparison_dir/final_${variant}_${trial}.stdout"
  done
done
