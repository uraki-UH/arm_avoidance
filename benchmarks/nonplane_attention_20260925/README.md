...
本機能は同日のユーザー指示で撤去済み。以下は削除前の評価記録。
旧`smoke.py`は削除前ソースとともに`artifacts/nonplane_attention_removal_20260925/source_before.tar.gz`へ退避。
現在の確認手順は[削除後検証](REMOVAL.md)を参照。共用の境界検索ベンチは保持。
