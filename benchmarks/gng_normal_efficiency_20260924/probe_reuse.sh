...
for voxel in 0.1 0.5; do
  timeout --signal=INT --kill-after=10s 180 taskset -c 0 python3 "$trial_root/benchmarks/gng_normal_efficiency_20260924/benchmark.py" \
    --library "$trial_artifacts/reuse/libgng_cpu.so" \
    ...
    --output "$trial_artifacts/reuse/probe_${voxel}.json" --frames 80 --warmup 20 --voxel "$voxel" \
    > "$trial_artifacts/reuse/probe_${voxel}.log" 2>&1
  echo "reuse voxel=$voxel probe complete"
done
