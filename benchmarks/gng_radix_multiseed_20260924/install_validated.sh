...
timeout 180 ctest --test-dir /ros2_ws/build/gng_cpu --output-on-failure > "$trial_artifacts/production_tests.log" 2>&1
...
output = Path('/ros2_ws/src/artifacts/gng_radix_multiseed_20260924')
summary = json.loads((output / 'summary.json').read_text())
assert len(summary['seeds']) == 6
assert all(case['compared_frames'] == 1200 for case in summary['conditions'].values())
assert all(case['graph_matches'] == 30 for case in summary['isolation'])
...
assert before_hash == '0dc97bf8914afda4ba015314891000febb90ffa271519291cbdee359d702fa3a', 'installed library changed externally'
shutil.copy2(installed, output / 'production_before.so')
# 稼働中プロセスの参照inodeを保つ、同一ディレクトリ内の原子的な差替え。
staged = installed.with_name('.libgng_cpu_radix_20260924.so')
...
PY
timeout 180 taskset -c 0 python3 "$trial_root/benchmarks/gng_production_efficiency_20260924/benchmark.py" \
