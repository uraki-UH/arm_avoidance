...
def main():
    ...
    if hasattr(lib, 'gng_get_normal_phase_ms'):
        lib.gng_get_normal_phase_ms.restype = ct.POINTER(ct.c_double)
    if hasattr(lib, 'gng_get_attention_phase_ms'):
        lib.gng_get_attention_phase_ms.restype = ct.POINTER(ct.c_double)
        lib.gng_get_attention_mapping.argtypes = [ct.POINTER(ct.c_uint32)]
        lib.gng_get_attention_mapping.restype = ct.c_void_p
    ...
    init_ms = (time.perf_counter_ns() - init_begin) / 1e6
    init_rss_kib = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    if args.enable_features:
    ...
    for frame_idx, (data, num_points, config) in enumerate(clouds):
        ...
        # ノードの入力対応の順序・個数・無効表現の照合。現行実装では全件null/0。
        node_members_digest = hashlib.sha256()
        for idx in range(result.node_num):
            value = result.nodes[idx]
            assert value.inpcl_num <= num_points
            assert value.inpcl_num == 0 or value.inpcl_ids
            node_members_digest.update(int(value.id).to_bytes(2, 'little'))
            node_members_digest.update(int(value.inpcl_num).to_bytes(4, 'little'))
            node_members_digest.update(bytes([bool(value.inpcl_ids)]))
            if value.inpcl_num:
                node_members_digest.update(ct.string_at(value.inpcl_ids, value.inpcl_num * 4))
        ...
        if hasattr(lib, 'gng_get_attention_phase_ms'):
            ...
            mapping_num = ct.c_uint32()
            mapping = lib.gng_get_attention_mapping(ct.byref(mapping_num))
            record['mapping_num'] = mapping_num.value
            record['mapping_sha256'] = hashlib.sha256(ct.string_at(mapping, mapping_num.value * 8)).hexdigest()
        record['normal_sha256'] = hashlib.sha256(nodes['normal'].tobytes()).hexdigest()
        record['rho_sha256'] = hashlib.sha256(nodes['rho'].tobytes()).hexdigest()
        record['positions_sha256'] = hashlib.sha256(nodes['pos'].tobytes()).hexdigest()
        if has_efficiency_view:
        ...
        if args.enable_features:
            ...
            record['delta_meta_sha256'] = hashlib.sha256(
                digest_structure(delta, ('nodes', 'edges'))).hexdigest()
    ...
    Path(args.output).write_text(json.dumps(dict(library=args.library, config=args.config, voxel=args.voxel,
        has_features=args.enable_features, warmup=args.warmup, init_ms=init_ms, init_rss_kib=init_rss_kib, peak_rss_kib=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, accepted=accepted, ignored=ignored,
