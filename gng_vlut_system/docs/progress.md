...
## 2026-09-26: GNGサンプラーの製品配布境界

- 開発用の外部登録権限と内部機能フラグを分離。把持・境界をライブラリへ組込み、製品SDKから任意規則・点番号・重みの入力APIを除外。[仕様・互換性](releases/2026-09-26_sampler_product.md)。
- 開発CPU25件・製品CPU22件、ROS関連4件／2件、SDKの拒否テストが成功。実bag960フレーム分で変更前後の出力一致、両構成のROS実行・製品ライブラリ実読込も確認。[測定・再現条件](../../benchmarks/sampler_product_20260926/README.md)。
- 開発install先を維持し、製品版は分離配置。試験プロセスを全終了し、既存bag・Viewer・コンテナを維持。

## 2026-09-26: セル評価基盤のコンパイル時拡張

- 属性集計・任意のファジィ評価／履歴を型で選択する共有基盤を追加し、GNGの把持・境界とFVGへ接続。既定OFFと既存判定を維持。[仕様・制約](releases/2026-09-26_voxel_framework.md)。
- OFF／全ON各CPU24件・ROS関連4件・中間3構成の実GNG APIが成功。実bag1,440フレームで出力一致、GNG本体機械語・構造体サイズも一致。ROS16フレームの候補一致・失効を確認。新しい本番評価式と認識品質は対象外。[条件・実測](../../benchmarks/voxel_framework_20260926/README.md)。
- 通常配布先はOFFで再ビルド、FVGはCOLCON_IGNOREを保持して分離ビルド。試験プロセスを終了し、既存bag・Viewer・コンテナを維持。

## 2026-09-25: CPU GNGのサンプリング共通化

- 既存入力セル・照合を共有する規則APIを追加し、把持・境界重点と学習配分を共通化。任意属性表と要求時だけのセル内ノード数集計を接続し、拡張方法を[仕様](../../ais_gng_cpu/docs/sampling.md)へ記録。非平面等の新しい本番評価式は未追加。2026-09-26に公開APIと規則生成を既存ヘッダーへ統合し、専用2ファイルを削減。
- 配布先2パッケージのビルド、CPU CTest 23対象、ROS単体4対象、実bag比較480フレームと失効検証16フレームが成功。[結果・互換性](releases/2026-09-25_common_sampling.md)。全体時間はほぼ同等、単純セル条件の追加コストを別測定。9月26日の統合後もビルド・同じ単体対象・実bag 16フレームが成功。追従性・認識精度は未検証。
- 全試験プロセス終了、既存GNG・bag・Viewerとコンテナ状態を維持。[起動コマンド・測定条件](../../benchmarks/common_sampling_20260925/README.md)。

## 2026-09-25: 非平面重点サンプリングの実装・評価・撤去

- 半径方式とセルスコア方式を実装・高速化し、出力20,000点と総学習4,000回を検証。ただし追加コストと合成入力での品質比較を踏まえ、同日のユーザー指示で両方式・専用点群・設定・APIを撤去。既存unknown・把持・境界重点、非平面成分抽出、通常エイジングを保持。[変更内容](releases/2026-09-25_nonplane_attention.md)。
- 削除前の[半径方式評価](../../benchmarks/nonplane_attention_20260925/README.md)・[voxel方式評価](../../benchmarks/voxel_attention_20260925/README.md)を過去記録として保持。測定条件・失敗・未検証は各記録に集約。削除前ソースはローカルの圧縮バックアップへ退避。
- 削除後は通常配布先2パッケージのビルド、CPU21対象、ROS単体3対象、実bag12フレーム×3条件で成功。非空Tmap・通常時の成分所属・廃止Publisherとパラメータの不在を確認。全試験プロセス終了、既存GNG・bag・Viewer・コンテナの維持をPIDで照合。[検証・起動コマンド](../../benchmarks/nonplane_attention_20260925/REMOVAL.md)。

## 2026-09-25: 大容量Markerの連続配信によるViewer更新待ちを抑制

- 平面ノードMarker約3.85 MB/フレームを確認し、反映通知と最新1件保持を追加。ノード・エッジ数を保持し、旧接続との互換性と共有辞書を維持。[仕様・検証](releases/2026-09-25_viewer_marker_stream.md)。
- 8更新の試験で反映待ち側は1件、従来方式側は8件。再開時の最新反映・Graph併用・全点数保持・再接続・回帰・ビルド成功。実画面FPSは未検証。全診断・試験プロセス終了、既存Viewerは停止せず次回再起動で適用。

## 2026-09-25: 交差点TFの位置・姿勢をYAML化

- Viewerと同じEuler XYZ順の`pos`・`rot_deg`から静的TFを配信する専用launchとYAMLを追加。[操作・仕様](../../ais_gng_cpu/README.md#交差点bagの位置姿勢をyamlで補正)。
- 分離ROSでTF連結と独立計算との行列一致、不正設定8件の拒否を確認。初回の通信不成立を含む[条件・結果](releases/2026-09-25_intersection_tf_yaml.md)。全試験プロセス終了。稼働中GNGへの適用・Viewer実描画は未検証。

## 2026-09-24: 静止した巨大平面のGNG密度制御を設計

- ユーザー指定の初期範囲に絞り、平面内部の学習配分・ノード追加抑制・段階削減と、境界・新規物体・連結性の保護を設計。[設計案と検証条件](designs/gng_static_plane_density.md)。
- `static_node`が長期残存フラグであること、平面面積・外周が未提供であること、平面抽出がGNG実行後であることをソースで確認。既存の保留記録を残りの対象範囲へ更新。文書差分・リンクを確認し、コード・設定変更、実測、ROSプロセスの起動停止はなし。

## 2026-09-24: ViewerのClusters描画負荷削減

- 個別メッシュ・速度矢印を一括描画へ変更し、選択無効時のイベント登録を除去。クラスタ生成とGNG学習は変更なし。[仕様・制限](releases/2026-09-24_viewer_cluster_rendering.md)。
- 回帰9件、Chromeの分離配置4条件の画像一致、lint、Frontendビルド成功。比較スクリプトの初回カメラ・イベント集計の誤りを修正し、実表示を確認した数値のみ採用。[条件・測定値・起動コマンド](../../benchmarks/viewer_clusters_20260924/README.md)。試験Chromeは全終了、既存ROS・bag・Viewerの停止操作なし。

## 2026-09-24: Matching.mdの既存ファジー照合経路を確認

- [Matching.md](../../Matching.md)から、テンプレート保存、`object_template_matching.launch.py`、既存matcher・validator・確定後の配信経路を確認。法線・rho・接続／平面支持・対応率・欠損率・反証評価を実装済み。環境座標への位置合わせは未実装。[現行仕様](TECHNICAL_SPEC.md#152-物体gngテンプレート照合と確定配信)。
- 調査時のコンテナでは既存matcher／validatorが未起動、既定の読込一覧には車両テンプレートなし。今回追加していた`viewer_vehicle_registration_node.py`は別実装であり、既存ファジー照合との接続は未実施。読取調査のみ。
...
## 2026-09-24〜25: 平面クラスタの整合化・局所スケール追従と分断抑制

- 面幅・保持・局所スケール・接触部照合、接続切れ保持・孤立猶予、未所属点の面内取り込み救済を修正。追加診断の1,170/93点の平面対を根拠に、1本接続統合の30点上限と設定を撤去、残差判定を共通化。相対点数条件の追加なし。既所属間の移動・誤統合防止条件は維持。[仕様・調査の限界](releases/2026-09-24_plane_cluster_consistency.md)。
- 同じ交差点GNG入力150フレーム・3試行で、点数上限撤去前後の平均クラスタ64.99→63.99、CPU時間12.075→12.205 ms、平面所属12,627.35→12,620.16点。最終93点の断片中87点が大面へ合流。OFF時の非時間集計一致を確認。先行比較・規模別測定も保持。高速化や実bag正解ラベルによる品質保証とは区別。[条件・結果](../../benchmarks/plane_consistency_20260924/README.md)。
- Release反映、関連133テストと分離ROSの出力確認が成功。追加2件中1件は修正前で失敗。1,170＋99点の3フレーム統合・段差拒否をROS出力でも確認。試験後の配布先バイナリ一致、試験ノード2つの正常終了を確認。既存ROS・bag・Viewerの停止なし。先行検証の失敗・並行ビルド後の再検証記録も保持。続報でテスト内の範囲for文2箇所をconst参照へ変更し、再ビルドの警告なし・平面53テスト成功を確認（`range_loop_build.log`・`range_loop_ctest.log`）。検証プロセスは終了、新規ROS起動なし。
...
- 取得済み5モデルの描画・操作・GPU解放、Node回帰7件を確認。追加でトラックを原本非変更のまま30万面・44.8 MBのテクスチャ内包GLBへ変換。Python回帰2件とTSXでの描画・寸法・画像保持・GPU解放、lint・Frontend/Backendビルド成功。[条件・誤差・再現手順](releases/2026-09-24_viewer_local_mesh.md)。
- 変換・検証用Chromeは全終了、専用プロファイル削除済み。既存Frontend・gateway・3コンテナを維持し、停止・再起動なし。原本OBJ・MTL・画像のハッシュ一致を確認。

## 2026-09-24: CPU GNGの探索本体を既存関数へ集約

- ユーザーの可読性に関する指摘に対応し、`getMinGrid()`・`getDownSamplingGrid()`へ本体を集約。別名の`*_impl`2関数と宣言を削除し、呼び出し側の名称・引数と探索開始時1回の参照先選択を維持。実装変更はこの2関数と内部宣言のみ。
- 840組で全照合項目一致。全体はvoxel 0.1 mで88.798→88.825 ms、0.5 mで32.562→31.847 ms。本番CTest21件＋API2件、WASM native1件、配布版の実入力30フレーム成功。Release反映済み。環境確認の`(deleted)`表記の誤判定を修正し、既存ROS11プロセス・3コンテナの維持と検証プロセス全終了、一時ビルド削除を確認。[条件・記録・起動コマンド](../../benchmarks/gng_search_readability_20260924/README.md)。

## 2026-09-24: CPU GNGの追加削減候補の参照調査

- `nodes_inpcl_ids`が確保のみで未参照（上限20万点で0.8 MB）、`voxel_rightshift_func`と旧乱数配列関数が本番コアから未呼出しと確認。[配列](../../ais_gng_cpu/src/gng_cpu/src/cpu/gng.hpp)、[旧補助関数](../../ais_gng_cpu/src/gng_cpu/src/utils/utils.cpp)。未呼出し関数の削除は実行時間短縮と区別。
- [出力生成](../../ais_gng_cpu/src/gng_cpu/src/cpu/gng.cpp)のエッジ事前計数と出力の二重走査、[入力ラベル](../../ais_gng_cpu/src/gng_cpu/src/cpu/voxel_grid.cpp)の全体ゼロ化後の範囲内点再書込み、[radix](../../ais_gng_cpu/src/gng_cpu/src/cpu/radix_sort.hpp)の固定4パス、[重み付き重点入力](../../ais_gng_cpu/src/gng_cpu/src/api.cpp)の重複検査用ソート後の入力順再コピーを確認。範囲外点の0ラベル・出力順と容量上限・同一セル内順・重複ID拒否の保持が削減条件。radixのパス省略は対象桁が全点で同じ場合に限定。今回は静的調査のみで、実装変更・速度比較・ROS起動なし。

## 2026-09-24: CPU GNGのソート用途確認と未使用対応表の削除

- 削除前の本番ソート7箇所を参照先まで追跡。入力基数ソートはセル区間・重心・元点ラベル・重点候補・観測に使用。クラスタ2箇所はID引継ぎ・検索、重点点IDは重複処理、乱数配列用2関数は本番内に呼出しなし。重心集計自体にはソートが必須ではないが、現在のセル対応実装では使用。
- 学習・通常出力に未使用の`voxel2node_ids`の確保・生成・整列を削除。全体はvoxel 0.1 mで93.319→88.383 ms、0.5 mで32.444→32.460 ms。0.5 mは対象区間のみ短縮し、全体改善は未確認。[条件・影響](releases/2026-09-24_gng_unused_mapping.md)。
- 840組の全照合項目一致、本番CTest21件＋API2件、WASM native1件、配布版の実入力30フレームが成功。Release版を反映済み。状態確認の自己誤検知を修正し、既存ROS11プロセス・3コンテナの維持、検証プロセス全終了・一時ビルド削除を確認。[根拠・起動コマンド](../../benchmarks/gng_unused_mapping_20260924/README.md)。

## 2026-09-24: 既存報告の書式統一と概要の短縮

- リリースノート191件を「要約」「条件・検証」へ整理。把持調査・過去の開発まとめの重複説明を短縮し、結果・条件・未検証範囲を保持。[文書一覧](README.md)。
- [報告テンプレート](RELEASE_NOTE_TEMPLATE.md)と作業規約へ短い書式を反映。同じ作業の続報は既存文書へ統合する方針を明記。実装・設定・計測データの変更なし。
- リリース本文1,352節とコードブロック184件の保持、Markdownリンク636件による新規リンク切れなし、差分書式の検証が成功。再計測・ROS起動なし。
