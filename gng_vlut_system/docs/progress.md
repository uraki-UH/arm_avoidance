...
- 面幅・保持・局所スケール・接触部照合・小断片救済を修正。2026-09-25に面外根拠がない非連結成分の所属維持・孤立猶予を追加。残存調査で1本接続521点・成長距離外341点を確認後、単一平面への短い面内接続を持つ未所属点だけの取り込み救済を追加。既所属間の移動条件は維持。追加診断で1,170/93点の平面対が幾何条件を通りながら、1本接続救済の上限30点で対象外となる事例を確認。この追加診断では実装・設定変更なし。[仕様・調査の限界](releases/2026-09-24_plane_cluster_consistency.md)。
- 同じ交差点GNG入力150フレーム・3試行で、今回の取り込み救済追加前後の平面所属11,615→12,627点、CPU時間12.220→11.958 ms。解放・分割回数は増加。OFF時の非時間集計一致と2,760〜44,160ノードの規模別比較を確認。全条件での高速化や実bagの正解ラベルによる品質保証とは区別。先行の面外分断追加前後の結果も保持。[条件・結果](../../benchmarks/plane_consistency_20260924/README.md)。
- Release反映、関連131テストと分離ROSの出力確認が成功。今回の先行追加4件中2件、前回の追加8件中6件は各修正前で失敗。単一接続・距離救済・面外形状拒否もROS出力で確認。並行ビルドによる配布先更新を検出後、現行バイナリを上書きせず全テスト・ROS試験を再実施して成功、試験前後のSHA-256一致を確認。失敗・並行更新の先行検証記録も保持。試験ノードは正常終了、既存ROS・bag・Viewerの停止なし。
