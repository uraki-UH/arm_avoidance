...
- 長い平面の面幅判定、生成・保持の整合化、局所中央値によるスケール追従、平面統計再利用を修正。続報で統合時の相互照合を接続端点へ限定し、小面の法線誤差の遠方外挿を撤去。各側の統合後RMS判定による段差・傾斜小面の分離を維持。[仕様・制限](releases/2026-09-24_plane_cluster_consistency.md)。
- 同じ交差点GNG入力150フレーム・3試行で、全域相互照合版からクラスタ107.51→80.68、平面所属60.5→60.2%、CPU時間13.374→13.094 ms。先行の面幅・保持・スケール比較は別保存。実画像の過剰分割全解消・誤統合率は未確認。[条件・結果](../../benchmarks/plane_consistency_20260924/README.md)。
- Release反映、関連113テストと分離ROSでの内部小面統合・段差分離が成功。新規の内部小面統合テストは変更前に失敗。CTest旧ライブラリ参照・ROS試験の配列初期化エラーを修正後に再検証。全試験プロセス停止、既存bag・gateway・GNG・平面ノードの同一PID稼働を確認。既存プロセスの停止操作なし。
