...
## 2026-09-25: 非平面連結成分の近傍重点サンプリング

- 5ノード以上の非平面成分近傍へ総学習回数の50%を配分するCPU機能を追加し、`at128.yaml`で有効化。旧unknown枠を置換し、小成分は通常学習と元データに保持。動静判定なし。[仕様・制約](releases/2026-09-25_nonplane_attention.md)。
- 同日、候補点群出力を有効化し、近傍検索を高速化。固定実入力・候補重み完全一致で追加選択処理20.219→11.977 ms、別3フレームで36〜48%短縮。ROS側22テスト・実bag6条件を通常配布先でも確認。初期CPU側2対象の検証を保持。試験側の再送による不一致は接続待ちへ修正。全試験プロセス終了、既存GNG・bag・Viewerの停止なし。[再現手順・限界](../../benchmarks/nonplane_attention_20260925/README.md)。
