...
## 2026-09-24: 平面クラスタの整合化・局所スケール追従と内部小面統合

- 長い平面の面幅判定、生成・保持の整合化、局所スケール追従、統計再利用と接続端点での相互照合を修正。続報で、小断片・短い接続・厳しい残差・3フレーム連続適合に限定した1本接続の救済を追加。通常の2本接続条件と段差分離を維持。[仕様・制限](releases/2026-09-24_plane_cluster_consistency.md)。
- 同じ交差点GNG入力150フレーム・3試行で、救済追加前後のクラスタ80.68→77.29、CPU時間12.575→12.573 ms、平面所属率ほぼ同等。最終フレームの小断片61→56。救済OFF時の非時間集計は変更前と一致。接続なしの断片は対象外、実画像との対応・正解ラベルによる誤統合率は未検証。[条件・結果](../../benchmarks/plane_consistency_20260924/README.md)。
- Release反映、関連118テストと分離ROSでの内部小面統合・段差分離・1本接続の3フレーム目の統合が成功。追加4テストは変更前に失敗。初回ROSビルドの整数型不一致を修正後に再検証。通常ビルド先の並行更新後も現行配布版で再試験成功。先行検証の失敗記録も保持。全試験・診断プロセス終了、既存ROSの停止操作なし。
