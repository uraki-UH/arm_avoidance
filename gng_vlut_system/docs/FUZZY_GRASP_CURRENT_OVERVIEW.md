...
本書は現行コードの調査資料。動作コードの変更なし。
今後のルール・入力・実装雛形・改善案は [把持ファジールール統合設計書](designs/fuzzy_grasp_design.md) を正本とし、本書は実装説明と検証結果を保持。
...
| [統合設計書・第3章](designs/fuzzy_grasp_design.md#rule-catalog) | R001〜R145の候補集 | 将来案。現行の実行ルール一覧ではない |
...
改善提案とROS統合案は [統合設計書の第5章](designs/fuzzy_grasp_design.md#open-design) へ移管。設計案を本資料へ重複記載しない。
...
- [計画GNG目標選択](../src/nodes/planning/topological_map_goal_selector_node.cpp)
...
- [将来のルール候補集](designs/fuzzy_grasp_design.md#rule-catalog)
