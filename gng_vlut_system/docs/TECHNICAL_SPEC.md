...
- 候補探索は開始ノードごとに安全な複数ゴールへの探索を共有。危険・衝突終点の例外許可と、実行系の隣接危険度ペナルティ有効時は個別探索。共有範囲は1回の計画内だけで、更新をまたぐ経路キャッシュなし。[比較検証](releases/2026-09-14_candidate_path_batch.md)。
- 複数の開始候補を扱う場合、1回の計画内で有向エッジの有効判定・基礎コストを遅延キャッシュ。安全制約・終点別ペナルティは各探索で評価。計画終了時に破棄し、開始候補が1つの場合はキャッシュ生成なし。
