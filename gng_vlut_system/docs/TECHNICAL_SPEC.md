...
元マップ座標系の`SpatialTree`をキャッシュし、候補が所属する到達セルを逆変換したAABBで検索。AABB内の点は従来のセル所属式で再確認するため、回転による外接箱の余分な点や上端境界は採用対象外。距離・姿勢・可操作性による順位付け、衝突ラベルの除外、同点時の元配列順は維持。単純な位置の最近傍N件への置換ではない。

索引は点の位置・配列順・件数が変わった場合に再構築。同じ座標の再配信、ラベル・ID・法線・headerのみの更新では再利用し、最新メッセージで評価。TF・到達グリッドの変更にも索引の再構築は不要。ROS受信メッセージを不変スナップショットとして扱い、非有限位置を索引から除外。比較・検証は[2026-09-15の記録](releases/2026-09-15_static_spatial_index.md)を参照。
