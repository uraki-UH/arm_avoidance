...
元マップ座標系の`bsp3d::Index<entry, double>`（葉32点、bboxなし）をキャッシュし、候補が所属する到達セルを逆変換したAABBで検索。AABB内の点は従来のセル所属式で再確認するため、回転による外接箱の余分な点や上端境界は採用対象外。距離・姿勢・可操作性による順位付け、衝突ラベルの除外、同点時の元配列順は維持。単純な位置の最近傍N件への置換ではない。

索引は元mapの配列添字ごとの座標を保持し、移動は`updatePosition`、有限点の追加・無効化・件数減少は`add`・`remove`で差分反映。`deque`により、件数変更時も残存要素のアドレスと木のハンドルを維持。座標変更を理由に索引全体を破棄する処理は廃止。bsp3d内部の分割・必要な部分木の再構成は継続。同じ座標の再配信、ラベル・ID・法線・headerのみの更新では空間索引を再利用し、最新メッセージで評価。TF・到達グリッドの変更にも再構築不要。ROS受信メッセージは不変スナップショットとして扱い、非有限位置を索引から除外。
...
- 候補専用ノードは固定GNGの有向隣接・関節コストを起動時に配列索引化。開始ノード別の再開可能な最短経路木を保持し、安全ゴール間で共有。危険・衝突終点の例外は終点別の独立木とし、別ゴールの通過許可への転用なし。未探索の独立木だけをCPU数までのワーカーで並列処理。[仕様・比較検証](releases/2026-09-14_candidate_static_path_index.md)。
- 通過可否が変化した場合は全探索木を失効。現在の開始候補・例外終点から外れた木は破棄。通過可否が同じ間は確定距離・親・探索キューを再利用し、未到達ゴールだけ探索を再開。候補評価式・同点時の選定順は維持。
- 固定コスト非対応の評価器や隣接危険度ペナルティ有効時は従来探索へフォールバック。その場合は1回の計画内だけで有向エッジ判定・基礎コストを遅延共有し、単一開始候補時は生成なし。固定索引使用時の重み・エッジ変更には`prepare_static_graph`の再実行が必要。現行候補専用ノードのモデルは読込後固定。
...
- 候補ロボット欄の「先頭N件表示」で受信済みinstanceの表示数を制限。既定0は全件。配信順の維持であり、現在の候補ID順を評価ランキングへ再計算しない。設定はViewerセッション内で保持し、ROS配信数・選定・計画には非適用。
- 「全件に戻す」で件数制限と単体選択を解除。「1体選択」では配信順の候補番号をスライダーで指定し、Visual・可操作性楕円体・Collisionの対象を一致。候補数減少時は表示番号を現在の範囲へ補正。候補IDによる同一個体の追跡ではない。
...
`output_topic`は非孤立の直接観測セルとedge補間セルの和集合をpublishする。異なるlabel間の補間セルは
近い端点側のlabelを継承する。

時間履歴を通過しても26近傍がない直接観測セルは、物体の連続形状を支持しないため把持候補の
主`output_topic`、edge・triangle補間の端点から除外する。削除はせず、表示・診断専用の
...
主`output_topic`の`Voxel`には単調増加する`revision`を付与し、更新ごとに全量snapshotを
transient localでpublishする。
...
主`output_topic`は非孤立の直接観測、edge補間、triangle面の和集合とする。重複セルは1件へ統合し、
直接観測labelを優先しながらedge・triangle支持数をsummaryへ残す。
...
Viewerは候補配列を直接受信し、同じIDのローカル`+Z`矢印を未評価=黄・範囲内=明るい薄緑水色・範囲外=灰で表示する。
この経路は物体ボクセルとグリッパ体積graphを必要とせず、上面把持対象の粗い選別に使う。
付属抽出は参照平面方式に固定。隣接する開口外平面からの符号付き離隔と入口エッジ長により、
候補平面に接続する非平面ノードを探索。平面OBB・TCPは変更せず、付属ノードの開口包含を評価。
他平面や`BOUNDARY_FREE_SPACE`の端点を経由する探索は除外。参照面がない場合は付属抽出を省略。
成分IDは同一フレーム内の集計に使用し、未分類IDだけを理由とした幾何的な除外はなし。
