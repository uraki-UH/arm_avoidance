...
**解決したい問題**: 手先位置が重複または近接するノードが多いため、緑・黄・赤が混ざってまだらになる。回転させると前後関係が入れ替わってちらつく。

**方針**: グラフ縮約による集約。格子(ボクセル)での集約は位相構造が消えて単なるオキュパンシーグリッドになるため採用しない。位相地図であることが手法の看板なので、そこを消す表現は本末転倒。

A. **グラフ縮約の実装 (C++ core)**
   coord エッジ(手先位置空間の近傍)を短い順に縮約する。union-find でメンバを管理。
   - 集約ノードの位置 = メンバの重心(代表ノードの選出ではなく集約)
   - 大きさ = 含む元ノード数(非一様な密度を保存)
   - 状態 = **best-wins の3値**
   - エッジ = 縮約で潰れなかった元エッジを継承(格子隣接からは引かない)
   - **マージ列(デンドログラム)を保存**し、任意のノード数で打ち切って任意レベルを取り出せるようにする
   *暫定目標: 5,500ノードから任意粒度のグラフを即座に取り出せる*

B. **集約結果のビューアへの配信**
   既存の `topofuzzy_bridge` / `gng_viewer_bridge` 経由。集約済みを送ることで通信量も減る。

C. **ビューア側の描画**
   ノード(球、大きさ=元ノード数、色=3値)とエッジ(線)。
   *暫定目標: 混色とちらつきが解消し、位相構造が見える*

D. **粒度スライダ**
   マージ列から任意レベルを取り出して切り替える。再計算不要。
...
- **集約規則は best-wins(最良値)**。worst-wins は「その場所の全姿勢が安全か」という誰も立てない問いに答えてしまい、把持対象の近傍が必ず赤くなって「赤いのに掴みに行く」矛盾が出る。プランナのゴール候補選定(`!node.status.is_colliding` で採用)とも基準が一致する。
- **粒度は可視化専用の要件にしない**。階層プランニング、通信量削減、協調動作エリアの抽出など別用途では基準が変わるため、粒度は引数として外から与える。
- `Status::is_surface` は使わない([TASK_CANDIDATES.md](./TASK_CANDIDATES.md) 参照)。

#### 未確認

- **ビューアがどちらの腕の手先位置を描いているか**。`weight_coord` は `coord_layer_index == 0` 固定で更新されるため、双腕なのに片腕分だけを描いている可能性がある。重なりの一因かもしれないので先に確認する。
- 目標ノード数の具体値。
