...
## 2026-09-25: 追加の非平面重点サンプリング全体

- 対象・判断: ユーザーの「じゃあ消して」により、voxel・radius両方式、セルスコア・履歴・専用API・関連設定・専用点群出力を撤去。既存unknown・把持・境界重点、非平面成分抽出、通常エイジングは維持。
- 根拠: 高速化後も実bagのノード内processingはON 63.803 ms、OFF 56.914 ms（10万入力点・完了2試行平均）。合成移動曲面でONの再現性向上も確認できず。方式一般の否定ではなく今回の追加実装に対する判断。
- 再検討条件: 低コスト方式と対象入力での品質改善を確認し、ユーザーが再導入を希望した場合。
- 関連資料: [撤去内容](releases/2026-09-25_nonplane_attention.md)、[比較条件・未検証](../../benchmarks/voxel_attention_20260925/README.md)。

## 2026-09-25: 重点非平面ノードのオクルージョン判定による追加削除

- 対象・判断: 今回追加した方向索引・可視性確認・確認履歴・削除加速と関連設定を撤去。この時点では重点サンプリングを継続したが、後に上記判断で撤去。通常のGNGエイジングは維持。
- 判断根拠: ユーザーの「オクルージョン判定おもいのでけして」という明示指示。固定100,000入力点・13,654セルの追加選択処理は確認なし2.317 ms、最大512候補確認込み7.783 ms。CPU 0固定・7回中央値の3試行中央値で、GNG学習・ROS配信・出力点群抽選は測定外（`performance_20k_matched.log`）。
- 再検討条件: より低コストな方法と必要な精度の評価、およびユーザーによる再導入指示。一般的な遮蔽判定の恒久禁止ではない。
- 関連資料: [仕様](../../ais_gng_cpu/docs/voxel_attention.md)、[検証記録](../../benchmarks/voxel_attention_20260925/README.md)。
