...
| 関節限界余裕 | 現行`joint_limit_margin_min/mean`［ノードスコア］ | A：評価値あり | 小・中・大 | 現行min/meanは同じ値。厳密な最小余裕・平均余裕は再定義・計算が必要 |
...
| 経路移動コスト | 現行`estimated_energy`＝各区間の関節変化量二乗和 | A：候補評価に出力 | 小・中・大 | **実消費エネルギー[J]ではない**。関節の単位・経路分割に依存 |
| 推定所要時間 | 現行`estimated_duration`［s］ | A：区間最大関節変化量／設定速度を積算 | 短・中・長 | 加速度・動力学を考慮した実行時間の保証ではない |
