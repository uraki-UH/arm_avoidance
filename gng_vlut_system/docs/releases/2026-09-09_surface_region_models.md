...
- 局所推定の列ピボット付きQR化、6×6行列での条件検査、悪条件時の全体SVD、反復早期終了と作業バッファ再利用。
- `surface_model.max_boundary_fits` の追加。既定32回、0で境界の追加フィットなし。
- 新規抽出時の平面・非平面混合統合に加え、追跡中の曲面にも接続・形状条件を満たす非平面ノードを付加。曲面係数の再推定なし。
- 追加点は当該フレームのみの所属。元の追跡支持が失われた場合、追加点だけによる延命なし。新規表示の元平面2クラスタ条件は維持。
- 保存カップ33フレームで、追跡なしの表示23→28フレーム、対象2平面の鋭い境界判定12→0フレーム。追跡ありの表示33フレームを維持、出現ID数5→1。
- 5回比較で、追跡込み平均0.751→0.648 ms、局所曲率0.252→0.177 ms。一方、追加検査により追跡なしは平均1.185→1.333 ms。ROS配送・描画は計測外。
