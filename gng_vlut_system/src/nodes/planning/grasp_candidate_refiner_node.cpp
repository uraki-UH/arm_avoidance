...
public:
  grasp_candidate_refiner():Node("grasp_candidate_refiner"),buffer_(get_clock()),listener_(buffer_)
  {
    ...
    timer_=create_wall_timer(std::chrono::duration<double>(1.0/update_hz),[this] { update(); });
...
private:
  ...
  void publish_markers(const output_array &out)
  {
    ...
    for (std::size_t idx=0;idx<out.candidates.size();++idx) {
      ...
      if (!std::isfinite(candidate.observed_width)) continue;
      marker line;
      line.header=out.header; line.ns="refined_contacts"; line.id=static_cast<int>(idx);
      line.type=marker::LINE_LIST; line.action=marker::ADD; line.pose.orientation.w=1;
      line.scale.x=0.003; line.color.a=1;
      line.color.r=candidate.has_observed_collision ? 1.0F:0.1F;
      line.color.g=candidate.has_observed_collision ? 0.2F:0.9F;
      line.color.b=candidate.has_joint_solution ? 0.1F:0.9F;
      if (candidate.has_contact_pair) {
        line.points={candidate.left_contact,candidate.right_contact}; array.markers.push_back(line);
      } else { line.color.r=line.color.g=line.color.b=0.6F; }
      line.ns="refined_width"; line.type=marker::TEXT_VIEW_FACING;
      line.points.clear(); line.scale.x=0; line.scale.z=0.015;
      line.pose.position=candidate.refined_pose.position; line.pose.position.z+=0.035;
      line.text="#"+std::to_string(candidate.source_candidate_id);
      if (candidate.has_contact_pair)
        line.text+=" width="+std::to_string(static_cast<int>(std::lround(candidate.contact_width*1000)))+" mm open="+
          std::to_string(static_cast<int>(std::lround(candidate.opening_width*1000)))+" mm ";
      else line.text+=" observed="+std::to_string(static_cast<int>(std::lround(candidate.observed_width*1000)))+" mm (unconfirmed) ";
      line.text+=candidate.reason;
  ...
  steady_clock::time_point source_received_,cloud_received_;
