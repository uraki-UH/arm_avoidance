...
Eigen::Isometry3d makeIsometry(const std::vector<double>& pos,
    ...
    T.linear() = (Eigen::AngleAxisd(rot_deg[2] * M_PI / 180.0, Eigen::Vector3d::UnitZ()) *
                  Eigen::AngleAxisd(rot_deg[1] * M_PI / 180.0, Eigen::Vector3d::UnitY()) *
                  Eigen::AngleAxisd(rot_deg[0] * M_PI / 180.0, Eigen::Vector3d::UnitX())).toRotationMatrix();
    return T;
}
...
public:
    ...
    : Node("topofuzzy_bridge_node") {
        ...
        if (!context_ || !context_->gng || !context_->mapper) {
            RCLCPP_ERROR(get_logger(), "Failed to load GNG/VLUT data from %s", gng_path.c_str());
            throw std::runtime_error("topofuzzy_bridge: failed to load safety context");
        }
        ...
        occupied_sub_ = create_subscription<std_msgs::msg::Int64MultiArray>(
            ...
            std::bind(&TopoFuzzyBridgeNode::occupiedVoxelCallback, this, std::placeholders::_1));
        danger_sub_ = create_subscription<std_msgs::msg::Int64MultiArray>(
            danger_voxels_topic_, 10,
            std::bind(&TopoFuzzyBridgeNode::dangerVoxelCallback, this, std::placeholders::_1));
        ...
        RCLCPP_INFO(get_logger(),
                    "topofuzzy_bridge_node initialized. edge_mode=%s occupied_topic=%s danger_topic=%s",
                    activeEdgeModeName(edge_mode_).c_str(),
                    occupied_voxels_topic_.c_str(),
                    danger_voxels_topic_.c_str());
...
private:
    ...
    int selectedEdgeMode() const {
        ...
        // Auto: prefer coord graph if it exists, otherwise fall back to angle graph.
    ...
    void annotateTopologyLocked() {
        ...
        topology_analyzer_.analyze(
            ...
            [&](int idx) -> const std::vector<int>& {
                return (mode == 0) ? gng.getNeighborsAngle(idx) : gng.getNeighborsCoord(idx);
            },
            true);
