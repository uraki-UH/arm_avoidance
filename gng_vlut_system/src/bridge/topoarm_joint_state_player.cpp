...
class TopoArmJointStatePlayer : public rclcpp::Node {
public:
  TopoArmJointStatePlayer()
  : Node("topoarm_joint_state_player") {
    ...
    RCLCPP_INFO(get_logger(),
                ...
                udp_port_, base_frame_.c_str(), output_topic_.c_str());
  }

  ~TopoArmJointStatePlayer() override { stopSocket(); }
...
private:
  void startSocket() {
    ...
    running_ = true;
    thread_ = std::thread(&TopoArmJointStatePlayer::receiverLoop, this);
  }
  ...
  void publishJointState(const std::string &csv) {
    ...
    for (int i = 0; i < 6; ++i) {
      msg.position[static_cast<std::size_t>(i)] =
          (raw_vals[static_cast<std::size_t>(i)] / 10.0) * kPi / 180.0;
    }
  ...
  std::string output_topic_ = "/joint_states_rviz";
  int sockfd_ = -1;
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<TopoArmJointStatePlayer>());
  rclcpp::shutdown();
  return 0;
}
