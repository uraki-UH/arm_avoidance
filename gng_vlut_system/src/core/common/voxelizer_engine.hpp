...
public:
    ...
    static void voxelizeMeshTriangles(const std::vector<Eigen::Vector3d>& vertices,
        ...
        for (size_t t = 0; t < vertices.size() / 3; ++t) {
            ...
            int s1 = std::max(1, static_cast<int>(std::ceil(d12 / step)));
            int s2 = std::max(1, static_cast<int>(std::ceil(d13 / step)));
            for (int i1 = 0; i1 <= s1; ++i1) {
                double t1 = static_cast<double>(i1) / s1;
                for (int i2 = 0; i2 <= std::max(0, static_cast<int>(s2 * (1.0 - t1))); ++i2) {
                    double t2 = static_cast<double>(i2) / s2;
                    Eigen::Vector3d p = v0 + t1 * (v1 - v0) + t2 * (v2 - v0);
                    vids.insert(grid.getFlatVoxelId(
                        ::common::geometry::VoxelUtils::worldToVoxel(p.cast<float>(),
                                                                      static_cast<float>(v_size))));
                }
            }
        }
