...
void GrowingNeuralGas2<T_angle, T_coord>::remove_node(int node) {
  if (node < 0 || (size_t)node >= nodes.size() || nodes[node].id == -1)
    return;
  // Should verify coord edges are empty too, or force clear them?
  // Since we don't remove nodes during coord training, force clear logic is
  // sufficient if called during cleanups.
...
bool GrowingNeuralGas2<T_angle, T_coord>::internalCheckPathColliding(
  ...
  // max_depth 6 は 2^6 =
  // 64分割相当（従来の50回スキャンをカバーしつつ早期終了可能）
...
void GrowingNeuralGas2<T_angle, T_coord>::one_train_update(
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    if (nodes[i].id == -1)
      continue;
    nodes[i].error_angle *= (1.0f - params_.beta);
  ...
  int n_search = std::min((int)candidates.size(), params_.n_best_candidates);
  std::partial_sort(candidates.begin(), candidates.begin() + n_search,
                    candidates.end());

  std::deque<int> candidate_ids;
  for (int i = 0; i < n_search; ++i)
    candidate_ids.push_back(candidates[i].second);
  ...
  float dist =
      std::sqrt(calc_squaredNorm_angle(sample_angle, nodes[s1].weight_angle));

  // --- 統計収集 (1次・2次遅れフィルタ) ---
  float dist_s1_sq =
      calc_squaredNorm_angle(sample_angle, nodes[s1].weight_angle);
  float dist_s2_sq =
      (s2 != -1) ? calc_squaredNorm_angle(sample_angle, nodes[s2].weight_angle)
                 : 0.0f;
...
bool GrowingNeuralGas2<T_angle, T_coord>::save(const std::string &filename) {
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    if (nodes[i].id != -1) {
      for (int n : edges_angle_per_node[i]) {
        if (i < n)
          edge_count++;
      }
    }
    ...
    if (nodes[i].id != -1) {
      for (int n : edges_angle_per_node[i]) {
        if (i < n) {
          ...
          ofs.write((char *)&edges_angle[i][n].age, sizeof(int));
          bool e_active = edges_angle[i][n].active;
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    if (nodes[i].id != -1) {
      for (int n : edges_coord_per_node[i]) {
        if (i < n)
          coord_edge_count++;
      }
    }
    ...
    if (nodes[i].id != -1) {
      for (int n : edges_coord_per_node[i]) {
        if (i < n) {
          ofs.write((char *)&i, sizeof(int));
          ofs.write((char *)&n, sizeof(int));
          ofs.write((char *)&edges_coord[i][n].age, sizeof(int));
          bool e_active = edges_coord[i][n].active;
          ofs.write((char *)&e_active, sizeof(bool));
        }
      }
    }
...
bool GrowingNeuralGas2<T_angle, T_coord>::load(const std::string &filename) {
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    ...
    for (int j = 0; j < (int)nodes.size(); ++j) {
      edges_angle[i][j] = EdgeInfo();
      edges_coord[i][j] = EdgeInfo();
    }
  }
