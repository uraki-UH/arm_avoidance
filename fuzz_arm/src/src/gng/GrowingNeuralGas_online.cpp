...
GrowingNeuralGas<T_angle, T_coord>::GrowingNeuralGas(
  ...
  edges_angle.assign(nodes.size(), std::vector<EdgeInfo>(nodes.size()));
  edges_coord.assign(nodes.size(), std::vector<EdgeInfo>(nodes.size()));
  ...
  for (int i = 0; i < params_.start_node_num; ++i) {
    ...
    for (int j = 0; j < (int)wa.size(); ++j) {
      wa(j) = static_cast<typename T_angle::Scalar>(rand()) /
              static_cast<typename T_angle::Scalar>(RAND_MAX);
    }
    ...
    for (int j = 0; j < (int)wc.size(); ++j) {
      wc(j) = static_cast<typename T_coord::Scalar>(rand()) /
              static_cast<typename T_coord::Scalar>(RAND_MAX);
    }
    add_node(wa, wc);
  }
}
...
int GrowingNeuralGas<T_angle, T_coord>::add_node(
  ...
  edges_angle_per_node[node_id] = std::unordered_set<int>();
  edges_coord_per_node[node_id] = std::unordered_set<int>();
...
void GrowingNeuralGas<T_angle, T_coord>::remove_node(int node) {
  ...
  edges_angle_per_node.erase(node);
  edges_coord_per_node.erase(node);
  nodes[node].id = -1;
...
void GrowingNeuralGas<T_angle, T_coord>::add_edge_angle(int node_1,
  ...
  if (node_1 < 0 || node_2 < 0)
    return;
  if (edges_angle[node_1][node_2].exists()) {
    edges_angle[node_1][node_2].age = 1;
    edges_angle[node_2][node_1].age = 1;
  } else {
    edges_angle_per_node[node_1].insert(node_2);
    edges_angle_per_node[node_2].insert(node_1);
    edges_angle[node_1][node_2].age = 1;
    edges_angle[node_2][node_1].age = 1;
  }
}
...
void GrowingNeuralGas<T_angle, T_coord>::remove_edge_angle(int node_1,
  ...
  if (node_1 < 0 || node_2 < 0)
    return;
  edges_angle_per_node[node_1].erase(node_2);
  edges_angle_per_node[node_2].erase(node_1);
  edges_angle[node_1][node_2] = EdgeInfo();
  edges_angle[node_2][node_1] = EdgeInfo();
}
...
void GrowingNeuralGas<T_angle, T_coord>::add_edge_coord(int node_1,
  ...
  if (node_1 < 0 || node_2 < 0)
    return;
  if (edges_coord[node_1][node_2].exists()) {
    edges_coord[node_1][node_2].age = 1;
    edges_coord[node_2][node_1].age = 1;
  } else {
    edges_coord_per_node[node_1].insert(node_2);
    edges_coord_per_node[node_2].insert(node_1);
    edges_coord[node_1][node_2].age = 1;
    edges_coord[node_2][node_1].age = 1;
  }
}
...
void GrowingNeuralGas<T_angle, T_coord>::remove_edge_coord(int node_1,
  ...
  if (node_1 < 0 || node_2 < 0)
    return;
  edges_coord_per_node[node_1].erase(node_2);
  edges_coord_per_node[node_2].erase(node_1);
  edges_coord[node_1][node_2] = EdgeInfo();
  edges_coord[node_2][node_1] = EdgeInfo();
}
...
bool GrowingNeuralGas<T_angle, T_coord>::save(
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    if (nodes[i].id == -1)
      continue;
    if (edges_angle_per_node.count(i) == 0)
      continue;
    for (int neighbor : edges_angle_per_node.at(i)) {
      if (i < neighbor) { // Save each edge only once
        angle_edge_count++;
      }
    }
  }
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    ...
    for (int neighbor : edges_angle_per_node.at(i)) {
      if (i < neighbor) {
        ...
        ofs.write((char *)&edges_angle[i][neighbor].age, sizeof(int));
      }
    }
  }
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    if (nodes[i].id == -1)
      continue;
    if (edges_coord_per_node.count(i) == 0)
      continue;
    for (int neighbor : edges_coord_per_node.at(i)) {
      if (i < neighbor) { // Save each edge only once
        coord_edge_count++;
      }
    }
  ...
  for (int i = 0; i < (int)nodes.size(); ++i) {
    ...
    for (int neighbor : edges_coord_per_node.at(i)) {
      if (i < neighbor) {
        ofs.write((char *)&i, sizeof(int));
        ofs.write((char *)&neighbor, sizeof(int));
        ofs.write((char *)&edges_coord[i][neighbor].age, sizeof(int));
      }
    }
  }
...
bool GrowingNeuralGas<T_angle, T_coord>::load(const std::string &filename) {
  ...
  edges_angle.assign(nodes.size(), std::vector<EdgeInfo>(nodes.size()));
  edges_coord.assign(nodes.size(), std::vector<EdgeInfo>(nodes.size()));
  edges_angle_per_node.clear();
  edges_coord_per_node.clear();
  std::queue<int> empty_queue;
  addable_node_indicies.swap(empty_queue);
  for (int i = 0; i < (int)nodes.size(); i++)
    addable_node_indicies.push(i);
