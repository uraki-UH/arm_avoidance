...
public:
  ...
  // 近傍取得
  const std::unordered_set<int> &getNeighborsAngle(int node_id) const {
    return edges_angle_per_node.at(node_id);
  }
  const std::unordered_set<int> &getNeighborsCoord(int node_id) const {
  ...
  bool isEdgeActive(int n1, int n2, int mode = 0) const {
    if (n1 < 0 || n2 < 0 || (size_t)n1 >= nodes.size() ||
      ...
      return false;
    if (mode == 0) {
      if (n1 >= (int)edges_angle.size() || n2 >= (int)edges_angle[n1].size())
        return false;
      return edges_angle[n1][n2].exists() && edges_angle[n1][n2].active;
    } else {
      if (n1 >= (int)edges_coord.size() || n2 >= (int)edges_coord[n1].size())
        return false;
      return edges_coord[n1][n2].exists() && edges_coord[n1][n2].active;
    }
  }
...
private:
  ...
  // Angle Space Edges
  std::unordered_map<int, std::unordered_set<int>> edges_angle_per_node;
  std::vector<std::vector<EdgeInfo>> edges_angle;

  // Coordinate Space Edges
  std::unordered_map<int, std::unordered_set<int>> edges_coord_per_node;
  std::vector<std::vector<EdgeInfo>> edges_coord;
