...
void SafetyStateManager::finalizeUpdate(float dt) {
  if (dt <= 0)
    return;
  for (int node_id : active_set_.indices()) {
...
void SafetyStateManager::updateDangerFieldFromPoints(
    std::shared_ptr<analysis::DenseSpatialIndex> spatial_index,
    const std::vector<Eigen::Vector3d> &points, float threshold,
    bool use_dilation) {
  if (!spatial_index || points.empty())
    return;

  debug_voxels_.clear();
  std::unordered_set<long> surface_voxels_set;
  for (const auto &p : points) {
    long vid = spatial_index->getVoxelId(p);
    surface_voxels_set.insert(vid);
  }

  std::vector<long> occupied_voxels(surface_voxels_set.begin(),
                                    surface_voxels_set.end());
  std::vector<long> danger_voxels;

  if (use_dilation && threshold > 1e-4) {
    if (std::abs(kernel_threshold_ - threshold) > 1e-4 ||
        std::abs(kernel_voxel_size_ - spatial_index->getResolution()) > 1e-6) {
      kernel_offsets_.clear();
      kernel_threshold_ = threshold;
      kernel_voxel_size_ = (float)spatial_index->getResolution();
      int range = std::ceil(threshold / kernel_voxel_size_);
      for (int x = -range; x <= range; ++x) {
        for (int y = -range; y <= range; ++y) {
          for (int z = -range; z <= range; ++z) {
            if (x == 0 && y == 0 && z == 0)
              continue;
            double dist = std::sqrt(x * x + y * y + z * z) * kernel_voxel_size_;
            if (dist <= threshold)
              kernel_offsets_.push_back(Eigen::Vector3i(x, y, z));
          }
        }
      }
    }

    std::unordered_set<long> danger_set;
    for (long vid : occupied_voxels) {
      Eigen::Vector3i center =
          GNG::Analysis::IndexVoxelGrid::getIndexFromFlatId(vid);
      for (const auto &offset : kernel_offsets_) {
        Eigen::Vector3i neighbor = center + offset;
        if ((neighbor.array() >= 0).all() &&
            (neighbor.array() < spatial_index->getDims().array()).all()) {
          danger_set.insert(
              GNG::Analysis::IndexVoxelGrid::getFlatVoxelId(neighbor));
        }
      }
    }
    danger_voxels.assign(danger_set.begin(), danger_set.end());
  }
  updateDangerFieldFromVoxels(spatial_index, occupied_voxels, danger_voxels);
}
...
void InstantaneousSafetyStrategy::update(SafetyStateManager &manager, float dt,
  ...
  for (int node_id : active_set.indices()) {
    float target_level = ::GNG::SafetyRules::deriveLevel(
...
void InstantaneousSafetyStrategy::updateDangerField(
  ...
  double margin = std::max(0.04, res * 1.2);
